...
@if (isset($announcement) && $announcement['status'] == 1)
    <div class="d-flex justify-content-between align-items-center" id="anouncement"
        ...
        <span class="ml-3 mr-3" style="font-size: 12px;cursor: pointer;color: darkred"
            onclick="myFunction()">X</span>
    </div>
@endif
            ...
            <div class="container ">
                ...
                <div class="input-group-overlay d-none d-md-block mx-4"
                    ...
                    <form action="{{ route('products') }}" type="submit" class="search_form">
                        ...
                        <diV class="card search-card"
                            style="position: absolute;background: white;z-index: 1001;width: 100%;display: none">
                ...
                <div class="navbar-toolbar d-flex flex-shrink-0 align-items-center">
                    <a class="navbar-tool navbar-stuck-toggler" href="#">
                     ...
                     <?php
                                /* if (auth('customer')->id()) {
                                    $count_service_enquiry = \App\Model\ServiceEnquiry::where('user_id', auth('customer')->id())
                                        ->where('status', 'open')
                                        ->count();
                                } else {
                                    $count_service_enquiry = 0;
                                } */
                            ?>
                    {{--
                        ...
                        <a class="navbar-tool-icon-box bg-secondary dropdown-toggle"
                            ...
                            <span class="navbar-tool-label">
                                <span class="countWishlist">{{ $count_service_enquiry ?? 0 }}</span>
                            </span>
                            <i class="navbar-tool-icon fa fa-gear"></i>
                        </a>
                    </div> --}}
                    <div class="navbar-tool dropdown {{ Session::get('direction') === 'rtl' ? 'mr-3' : 'ml-3' }}">
                    ...
                    @if (auth('customer')->check())
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                <a class="dropdown-item" href="{{ route('account-services') }}">
                                    {{ \App\CPU\translate('my_services') }}</a>
            ...
            <div class="container">
                <div class="collapse navbar-collapse" id="navbarCollapse"
                    ...
                    <div class="input-group-overlay d-md-none my-3">
                        <form action="{{ route('products') }}" type="submit" class="search_form mx-4">
                            <input class="form-control appended-form-control search-bar-input-mobile mx-2" type="text"
                                autocomplete="off" placeholder="{{ \App\CPU\translate('search') }}" name="name">
                            <input name="data_from" value="search" hidden>
                            <input name="page" value="1" hidden>
                            <button class="input-group-append-overlay search_button" type="submit"
                    ...
                    @php(
    $categories = \App\Model\Category::with(['childes.childes'])->where('position', 0)->paginate(25))
                    <ul
                    ...
                    <ul class="navbar-nav"
                        ...
                        @php(
    $service = App\Model\Product::with(['reviews'])->active()->where('product_type', 'service')->count())
                        @if ($service > 0)
                            <li class="nav-item dropdown {{ request()->is('/') ? 'active' : '' }}">
                                <a class="nav-link text-capitalize"
                                    href="{{ route('products', ['product_type' => 'service', 'page' => 1]) }}">{{ \App\CPU\translate('services') }}</a>
                            </li>
                        @endif
                        @php(
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton"
                                            ...
                                            <a class="dropdown-item" href="{{ route('shop.apply') }}">
                                                <b>{{ \App\CPU\translate('Become a') }}
                                                    {{ \App\CPU\translate('Seller') }}</b>
                                            </a>
                                            <div class="dropdown-divider"></div>
                                            <a class="dropdown-item" href="{{ route('seller.auth.login') }}">
                                                <b>{{ \App\CPU\translate('Seller') }}
                                                    {{ \App\CPU\translate('login') }} </b>
                                            </a>
                                        </div>
                                    </div>
                                </li>
