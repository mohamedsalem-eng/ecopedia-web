...
@section('content')
    @php($area_list = \App\CPU\BackEndHelper::area_list())
                                    ...
                                    @foreach ($order->details as $key => $detail)
                                        <p>
                                            <a data-toggle="collapse" href="#collapse-{{ $key }}" role="button"
                                                ...
                                                {{ App\CPU\Helpers::get_exceprt(@$detail->product?->name ?? '', 70, '...') }}
                                            </a>
                                        </p>
                                        <div class="collapse" data-parent='#product-group'
                                            id="collapse-{{ $key }}">
                                            <div class="card card-body">
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-1 text-nowrap"><strong
                                                            class="text-dark">{{ \App\CPU\translate('available') }}</strong>
                                                    </div>
                                                </div>
                                                @if (empty($detail->supplier))
                                                    @foreach (@$detail->product?->suppliers ?? [] as $key => $supplier)
                                                        <div class="row py-1">
                                                            <div class="col-sm-3">
                                                                <span class="text-dark">
                                                                    {{ App\CPU\Helpers::get_exceprt($supplier->name, 70, '...') }}
                                                                </span>
                                                            </div>
                                                            <div class="col-sm-3">
                                                                <span class="text-dark">
                                                                    {{ App\CPU\Helpers::get_exceprt($supplier->address, 70, '...') }}
                                                                </span>
                                                            </div>
                                                            <div class="col-sm-3">
                                                                <ul>
                                                            ...
                                                            <div class="col-sm-2">
                                                                {{ $supplier->pivot->points ?? '0' }}
                                                            </div>
                                                            <div class="col-sm-1">
                                                                <input type="checkbox" data-detail="{{ $detail->id }}"
                                                                    value="{{ $supplier->id }}" name="supplier_id">
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                @else
                                                    <div class="row py-1">
                                                        <div class="col-sm-3">
                                                            <span class="text-dark">
                                                                {{ App\CPU\Helpers::get_exceprt(@$detail->supplier?->name ?? '', 70, '...') }}
                                                            </span>
                                                        </div>
                                                        <div class="col-sm-3">
                                                            <span class="text-dark">
                                                                {{ App\CPU\Helpers::get_exceprt(@$detail->supplier?->address ?? '', 70, '...') }}
                                                            </span>
                                                        </div>
                                                        <div class="col-sm-3">
                                                            <ul>
                                                                @foreach (@$detail->supplier?->phones ?? [] as $phone)
                                                                    <li>
                                                                        {{ $phone }}
                                                                    </li>
                                                                @endforeach
                                                        ...
                                                        <div class="col-sm-2">
                                                            {{ @$detail->product?->suppliers?->firstWhere('id', $detail->supplier_id)?->pivot?->points ?? '0' }}
                                                        </div>
                                                        <div class="col-sm-1">
                    ...
                    @if ($order->customer)
                        <div class="card-body">
                            ...
                            <span class="d-block">{{ \App\CPU\translate('Name') }} :
                                ...
                                <strong>{{ $shipping_address ? ( $area_list->firstWhere('id', $shipping_address->city)['name'] ?? '' )  : \App\CPU\translate('empty') }}</strong><br>
                            ...
                            <span class="d-block">{{ \App\CPU\translate('Name') }} :
                                ...
                                {{ \App\CPU\translate('City') }}:
                                <strong>{{ $billing ? ( $area_list->firstWhere('id',  $billing->city)['name'] ?? '' ) : \App\CPU\translate('empty') }}</strong><br>
                                {{ \App\CPU\translate('zip_code') }} :
...
@push('script_2')
    <script>
        $('input[name="supplier_id"]').on('click', function() {
            const supplier_id = $(this).val();
            const detail_id = $(this).data('detail');
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{ route('admin.orders.supplier') }}",
                        method: 'POST',
                        data: {
                ...
                google.maps.event.addListener(marker, 'click', (function(marker) {
                        return function() {
                            infowindow.setContent(
                                "<div style='float:left'><img style='max-height:40px;wide:auto;'
                                src =
                                '{{ asset('storage/app/public/profile/') }}{{ $order->customer->image ?? '' }}' >
                                < /div> <
                        ...
                        })(marker)); locationbounds.extend(marker.getPosition());
                @endif
