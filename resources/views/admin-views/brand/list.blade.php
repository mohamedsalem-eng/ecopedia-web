...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-2">
            <h1 class="h3 mb-0 text-black-50">{{ \App\CPU\translate('brand_list') }} <span
                        ...
                        <form action="{{ url()->current() }}" method="GET">
                            <div class="input-group input-group-merge input-group-flush">
                                ...
                                <input id="datatableSearch_" type="search" name="search" class="form-control"
                                    placeholder="{{ \App\CPU\translate('Search') }} {{ \App\CPU\translate('Brands') }}"
                            ...
                            <table style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                                ...
                                <thead class="thead-light">
                                    <tr>
                                        ...
                                        <th scope="col">{{ \App\CPU\translate('is_api') }}</th>
                                    ...
                                    @foreach ($br as $k => $b)
                                        <tr>
                                            ...
                                            <td><label class="switch">
                                                    <input type="checkbox" onclick="change_api('{{ $b['id'] }}')"
                                                        {{ $b->is_api == 1 ? 'checked' : '' }}>
...
@push('script')
    <script>
        function change_api(id) {
            ...
            $.ajax({
                url: "{{ route('admin.brand.change-api') }}/" + id,
                method: 'POST',
                data: {
                    brand: id
                },
                success: function() {
                    toastr.success('{{ \App\CPU\translate('Brand updated successfully') }}');
                }
            });
        }
