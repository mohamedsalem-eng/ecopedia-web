    ...
    <div class="content container-fluid">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item" aria-current="page">{{ \App\CPU\translate('service_enquiry') }}</li>
            </ol>
        </nav>
                    ...
                    <div class="card-header">
                        ...
                        <div class="flex-between row justify-content-between align-items-center flex-grow-1 mx-1">
                            <div class="flex-start">
                                ...
                                <div class="mx-1">
                                    <h5 style="color: red;">({{ $enquiries->total() }})</h5>
                                </div>
                            </div>
                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-merge input-group-flush">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                            placeholder="{{ \App\CPU\translate('Search by Subject') }}"
                                ...
                                <tbody>
                                    @foreach ($enquiries as $key => $enquiry)
                                        <tr class="text-center">
                                            ...
                                            <td>{{ $enquiry->user->display_name }}</td>
                                            <td><label class="switch"><input type="checkbox" class="status"
                                                        id="{{ $enquiry->id }}"
                                                        @if ($enquiry->status == 'open') checked @endif><span
                                                        class="slider round"></span></label></td>
                                            <td>
                                                <a href="{{ route('admin.service-enquiry.single-enquiry', $enquiry['id']) }}"
        ...
        @push('script')
            ...
            <script>
                $(document).on('change', '.status', function() {
                    ...
                    $.ajax({
                        url: "{{ route('admin.service-enquiry.status') }}",
                        method: 'POST',
                        data: {
                            id: id,
                            status: status
