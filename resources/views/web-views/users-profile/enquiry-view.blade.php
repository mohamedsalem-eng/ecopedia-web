...
@section('title', \App\CPU\translate('service_form'))
        ...
        <div class="row">
            ...
            <div class="col-md-9 sidebar_heading">
                <h1 class="h3  mb-0 float-{{ Session::get('direction') === 'rtl' ? 'right' : 'left' }} headerTitle">
                    {{ \App\CPU\translate('Service Form') }}</h1>
            </div>
        </div>
    </div>
    <!-- Page Content-->
        ...
        <div class="row">
            ...
            <section class="col-lg-8">
                ...
                <div class="d-none d-lg-flex justify-content-between align-items-center pt-lg-3 pb-4 pb-lg-5 mb-lg-4">
                    <div
                        class="d-flex w-100 text-light text-center {{ Session::get('direction') === 'rtl' ? 'ml-3' : 'mr-3' }}">
                        <div class="font-size-ms px-3">
                            <div class="font-weight-medium text-dark">{{ \App\CPU\translate('Date Submitted') }}</div>
                            <div class="text-dark">
                                {{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $enquiry->created_at)->format('Y-m-d') }}
                            </div>
                        </div>
                        <div class="font-size-ms px-3">
                            <div class="font-weight-medium text-dark">{{ \App\CPU\translate('Last Updated') }}</div>
                            <div class="text-dark">
                                {{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $enquiry->updated_at)->format('Y-m-d') }}
                            </div>
                        </div>
                        <div class="font-size-ms px-3">
                            <div class="font-weight-medium" style="color:black">{{ \App\CPU\translate('service_name') }}
                            </div>
                        ...
                        <div class="font-size-ms px-3">
                            ...
                            @if ($enquiry->status == 'open')
                                <span class="badge badge-secondary">{{ $enquiry->status }}</span>
                            @else
                                <span class="badge badge-secondary">{{ $enquiry->status }}</span>
                            @endif
                        </div>
                ...
                <div class="card pt-2">
                    ...
                    <!-- enquiry details (visible on mobile)-->
                    <div class="d-flex d-lg-none flex-wrap bg-secondary text-center rounded-lg pt-4 px-4 pb-1 mb-4">
                        ...
                        <div class="font-size-sm px-3 pb-3">
                            ...
                            @if ($enquiry['status'] == 'open')
                                <span class="badge btn btn-secondary">{{ $enquiry->status }}</span>
                            @else
                                <span class="badge btn btn-secondary">{{ $enquiry->status }}</span>
                            @endif
                        </div>
                    ...
                    <div class="media pb-4 flex-row-reverse for-margin-sms">
                        <img class="rounded-circle mx-1"
                            style="text-align: {{ Session::get('direction') === 'rtl' ? 'left' : 'right' }}; height:40px; width:40px;"
                            onerror="this.src='{{ asset('assets/back-end/img/user.png') }}'"
                            src="{{ asset('storage/app/public/profile') }}/{{ auth('customer')->user()->image }}"
                            alt="{{ auth('customer')->user()->f_name }}" />
                        <div
                            class="media-body flex-column d-flex pl-3 justify-content-end align-items-end align-content-end {{ Session::get('direction') === 'rtl' ? 'pr-3' : 'pl-3' }}">
                            <h6 class="font-size-md mb-2 font-weight-bold">{{ auth('customer')->user()->f_name }}</h6>
                            ...
                            <span class="font-size-ms text-muted">
                                ...
                                {{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $enquiry->created_at)->format('Y-m-d h:i A') }}
                            </span>
                        </div>
                    </div>
                    @foreach ($enquiry->conversations as $conversation)
                        ...
                        @if ($conversation['customer_message'] == null)
                            <div class="media pb-4 ">
                                <div class="media-body  {{ Session::get('direction') === 'rtl' ? 'pr-3' : 'pl-3' }}">
                                    ...
                                    <h6 class="font-size-md mb-2 font-weight-bold">{{ $admin->name }}</h6>
                        ...
                        @if ($conversation->admin_message == null)
                            <div class=" media pb-4 for-margin-sms flex-row-reverse">
                                <img class="rounded-circle mx-1" height="40" width="40"
                                    onerror="this.src='{{ asset('assets/back-end/img/user.png') }}'"
                                    src="{{ asset('storage/app/public/profile') }}/{{ auth('customer')->user()->image }}"
                                    alt="{{ auth('customer')->user()->f_name }}" />
                                <div
                                    class="media-body flex-column d-flex pl-3 justify-content-end align-items-end align-content-end {{ Session::get('direction') === 'rtl' ? 'pr-3' : 'pl-3' }}">
                                    <h6 class="font-size-md font-weight-bold mb-2">{{ auth('customer')->user()->f_name }}</h6>
                ...
                <div class="col-sm-12">
                    ...
                    <form class="needs-validation" href="{{ route('service-form.comment', [$enquiry->id]) }}"
                        ...
                        <div class="d-flex flex-wrap justify-content-between align-items-center">
                            <div class="">
                                <a href="{{ route('service-form.close', [$enquiry->id]) }}" class="btn btn-secondary"
