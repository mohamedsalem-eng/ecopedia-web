...
@section('content')
    ...
    {{-- flash deal --}}
    @php(
    $flash_deals = \App\Model\FlashDeal::with(['products.product'])->where(['status' => 1])->where(['deal_type' => 'flash_deal'])->whereDate('start_date', '<=', date('Y-m-d'))->whereDate('end_date', '>=', date('Y-m-d'))->first())
                ...
                @foreach ($brands as $brand)
                    <div class="text-center">
                        <a href="{{ route('products', ['id' => $brand['id'], 'data_from' => 'brand', 'page' => 1]) }}">
                            <div class="brand_div d-flex align-items-center justify-content-center" style="height:100px">
    ...
    {{-- featured deal --}}
    @php(
    $featured_deals = \App\Model\FlashDeal::with(['products.product.reviews'])->where(['status' => 1])->where(['deal_type' => 'feature_deal'])->first())
            ...
            <div class="row">
                <div class="col-xl-3 col-md-4 right">
                    <div class="d-flex align-items-center justify-content-center featured_deal_left">
                        <h1 class="featured_deal_title" style="padding-top: 12px">
                            {{ \App\CPU\translate('featured_deal') }}</h1>
                    </div>
                </div>
        ...
        <div class="row">
            ...
            <div class="col-xl-3 col-md-4 pb-4 mt-3">
                <div class="deal_of_the_day">
                    ...
                    @else
                        @php($product = \App\Model\Product::active()->where("product_type","product")->inRandomOrder()->first())
                        @if (isset($product))
                    ...
                    <div class="row p-0">
                        <div class="col-md-3 p-0 text-center mobile-padding mt-1 mt-md-0">
                            ...
                            <div class="deal-title">3 {{ \App\CPU\translate('days') }}
                                <br><span>{{ \App\CPU\translate('delivery') }}</span>
                            </div>
                        </div>
            ...
            <div class="col-xl-9 col-md-8">
                <div class="section-header">
                    ...
                    <div>
                        <a class="btn btn-outline-accent btn-sm viw-btn-a"
                            href="{{ route('products', ['data_from' => 'latest', "product_type" => "product"]) }}">
                            {{ \App\CPU\translate('view_all') }}
                            <i
                                class="czi-arrow-{{ Session::get('direction') === 'rtl' ? 'left mr-1 ml-n1' : 'right ml-1 mr-n1' }}"></i>
                        </a>
            ...
            <div class="col-12">
                <div class="section-header">
                    <div class="feature_header">
                        <span class="for-feature-title">{{ \App\CPU\translate('latest_service') }}</span>
                    </div>
                    <div>
                        <a class="btn btn-outline-accent btn-sm viw-btn-a"
                            href="{{ route('products', ['data_from' => 'latest', "product_type" => "service"]) }}">
                            {{ \App\CPU\translate('view_all') }}
                            <i
                                class="czi-arrow-{{ Session::get('direction') === 'rtl' ? 'left mr-1 ml-n1' : 'right ml-1 mr-n1' }}"></i>
                        </a>
                ...
                <div class="row mt-2 mb-3">
                    @foreach ($latest_services as $service)
                        <div class="col-xl-2 col-sm-3 col-6 mb-2">
                            @include('web-views.partials._single-product', ['product' => $service])
                        </div>
                    @endforeach
                </div>
            </div>
                    ...
                    <div class="category_div" style="height: 132px; width: 100%;">
                        <a
                            ...
                            <img style="vertical-align: middle; padding: 16%;height: 100px"
                                ...
                                alt="{{ $category->name }}">
                            <p class="text-center small" style="margin-top: -20px">{{ Str::limit($category->name, 17) }}
                            </p>
                        </a>
                    </div>
                @endforeach
