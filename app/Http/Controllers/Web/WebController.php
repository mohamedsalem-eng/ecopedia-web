...
class WebController extends Controller
{
    ...
    {
        //return view("web-views.invoice", ["order" => Order::find("100014")]);
/* $test = Category::with(['childes' => fn ($q) => $q->withoutGlobalScope("home_status"), "childes.childes" => fn($q) => $q->withoutGlobalScope("home_status")])->where('position', 0)->first();
dd($test); */
        ...
        $latest_products = Product::with(['reviews'])->where("product_type", "product")->active()->orderBy('id', 'desc')->take(8)->get();
        $latest_services = Product::with(['reviews'])->where("product_type", "service")->active()->orderBy('id', 'desc')->take(8)->get();
        ...
        return view('web-views.home', compact('featured_products', 'latest_services', 'topRated', 'bestSellProduct', 'latest_products', 'categories', 'brands', 'deal_of_the_day', 'top_sellers', 'home_categories'));
    }
    ...
    {
        $category = Category::with(['childes.childes'])->withoutGlobalScope("home_status")->where('id', $id)->first();
    ...
    public function checkout_details(Request $request)
    {
        ...
        if (count($cart_group_ids) > 0) {
            ...
            return view('web-views.checkout-shipping', compact('area_array'));
        }
    ...
    {
        $cart_group_ids = CartManager::get_cart_group_ids();
        $cart_count = CartShipping::whereIn('cart_group_id', $cart_group_ids)->count();
        /* if ($cart_count != count($cart_group_ids)) {
            Toastr::info(translate('select_shipping_method_first'));
            return redirect('shop-cart');
        } */
        ...
        if (session()->has('address_id') && count($cart_group_ids) > 0) {
            ...
            $shippings = \App\CPU\Helpers::get_shipping_methods(1, 'admin');
            $address = ShippingAddress::find(session("address_id"), 'city');
            $cost = BackEndHelper::area_list()->firstWhere("id", $address->city)['cost'] ?? 0;
            //dd($shippings);
            $shippings->map(function ($method) use ($cost) {
                if ($method->has_area) {
                    $method->cost += $cost;
                }
                return $method;
            });
            return view('web-views.checkout-payment', compact('shippings'));
        }
    ...
    public function seller_shop(Request $request, $id)
    {
        ...
        foreach ($category_ids as $category_id) {
            $category = Category::with(['childes.childes'])->withoutGlobalScope("home_status")->where('position', 0)->find($category_id);
            if ($category != null) {
    ...
    public function product($slug)
    {
        ...
        if ($product != null) {
            ...
            $relatedProducts = Product::with(['reviews'])->active()
                ...
                ->when($product->product_type == 'service', function ($q) {
    ...
    public function products(Request $request)
    {
        ...
        if ($request['product_type'] == 'service') {
            $query = $porduct_data->where('product_type', 'service');
        } elseif ($request['product_type'] == 'product') {
            $query = $porduct_data->where('product_type', 'product');
        }
        ...
        if ($request['data_from'] == 'category') {
            $data['brand_name'] = Category::withoutGlobalScope("home_status")->find((int)$request['id'])->name;
        }
        if ($request['data_from'] == 'brand') {
    ...
    public function contact_refresh(Request $request)
    {
