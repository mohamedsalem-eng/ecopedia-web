...
class BusinessSettingsController extends Controller
{
    ...
    public function updateInfo(Request $request)
    {
        ...
        if ($request->has('payment_logos')) {
            $paymentOldLogos = Helpers::get_business_settings("payment_logos");

            $logos = collect($request->payment_logos)->mapWithKeys(function ($logo, $key) use ($paymentOldLogos) {

                if (is_string($logo)) {
                    return [$key => $logo];
                }
                return [$key => ImageManager::update('company/', @$paymentOldLogos[$key], 'png', $logo)];
            });

            DB::table('business_settings')->updateOrInsert(['type' => 'payment_logos'], [
                'value' => json_encode($logos->flatten()->values()),
            ]);
        }
