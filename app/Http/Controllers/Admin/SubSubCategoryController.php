...
class SubSubCategoryController extends Controller
{
    public function index(Request $request)
    {
        ...
        $categories = $categories->withoutGlobalScope('home_status')->latest()->paginate(Helpers::pagination_limit())->appends($query_param);
    ...
    {
        $data = Category::where('id', $request->id)->withoutGlobalScope('home_status')
            ->first();
    ...
    {
        if ($request->ajax()) {
            $data = Category::where('position', 2)->withoutGlobalScope('home_status')->orderBy('id', 'desc')->get();
            return response()->json($data);
        }
    }
    ...
    {
        $data = Category::where("parent_id", $request->id)->withoutGlobalScope('home_status')->get();
        $output = "";
        foreach ($data as $row) {
    ...
    {
        $data = Category::where('id', $request->id)->withoutGlobalScope('home_status')->first();
        return response()->json($data);
    }
}
