...
class CurrencyController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $currency->save();
        session()->forget("currency_translation");
    ...
    {
        $data = Currency::withoutGlobalScopes()->where("id",$id)->with("translations")->first();
    ...
    public function update(Request $request, $id)
    {
        ...
        $currency->name = $request->name;
        //$currency->symbol = $request->symbol;
        $currency->symbol = $request->symbol[array_search('en', $request->lang)];
        ...
        foreach ($request->lang as $index => $key) {
            if ($request->symbol[$index] && $key != 'en') {
                array_push($data, array(
                    'translationable_type' => 'App\Model\Currency',
                    'translationable_id' => $currency->id,
                    'locale' => $key,
                    'key' => 'symbol',
                    'value' => $request->symbol[$index],
                ));
            }
        }
    ...
    public function delete($id)
    {
        if (!in_array($id, [1, 2, 3, 4, 5])) {
            ...
            $translation = Translation::where('translationable_type', 'App\Model\Currency')
