...
use App\Model\{DeliveryMan, Order, OrderDetail, OrderTransaction, Seller,  ShippingAddress, Supplier, TraceOrder};
...
class OrderController extends Controller
{
    ...
    {
        $order = Order::with('details.product.suppliers', 'shipping', 'shippingAddress', 'seller')->where(['id' => $id])->first();
    ...
    public function add_delivery_man($order_id, $delivery_man_id)
    {
        ...
        TraceOrder::recordUpdate($order, null, 'add_delivery_man', $delivery_man_id);
    ...
    public function status(Request $request)
    {
        ...
        TraceOrder::recordUpdate($order, $request, 'order_status');
    ...
    public function payment_status(Request $request)
    {
        if ($request->ajax()) {
            $order = Order::find($request->id);
            TraceOrder::recordUpdate($order, $request, 'payment_status');
    ...
    {
        if ($request->ajax()) {
            $supplier = Supplier::where("id", $request->supplier_id)->select("id")->first();
            if (!$supplier) {
                return response()->json([\App\CPU\translate("Supplier is not correct")], 404);
            }

            $orderDetail = OrderDetail::where("id", $request->detail_id)->first();
            if (!$orderDetail) {
                return response()->json([\App\CPU\translate("Product is not correct")], 404);
            }
            $orderDetail->supplier_id = $request->supplier_id;
            //$orderDetail->product->suppliers()->where("supplier_id", $request->supplier_id)->increment('points');
            debug(now());
            $orderDetail->product->suppliers()->where("supplier_id", $request->supplier_id)
                ->update([
    ...
    public function generate_invoice($id)
    {
        ...
        $data["order"] = $order;
        $mpdf_view = view('admin-views.order.invoice')->with('order', $order)->with('seller', $seller);
