...
class ProductController extends BaseController
{
    public function add_new()
    {
        ...
        return view('admin-views.product.add-new', compact('cat', 'br', 'suppliers'));
    }
    ...
    {
        $is_supplier = in_array('supplier_management', (array)json_decode(auth("admin")->user()->role->module_access)) == true;
        $validator = Validator::make($request->all(), [
            ...
            'purchase_price' => 'required|numeric|min:',
        ...
        if ($request->ajax()) {
            ...
            if ($request->has('supplier_ids')) {
                $p->suppliers()->attach($request->supplier_ids);
            }
            ...
            if ($is_supplier) {
                ...
                return redirect()->route('admin.supplier-management.list', ['supplier']);
            }
        }
    }
    ...
    {
        $is_supplier =
            in_array('supplier_management', (array)json_decode(auth("admin")->user()->role->module_access)) == true;
        $query_param = [];
        $search = $request['search'];
        if ($is_supplier) {
            $pro = Product::where(['user_id' => auth("admin")->id()]);
        } else {
        ...
        /* else {
            ...
            if ($business_mode != 'multi')
                return redirect(route('admin.dashboard')); //remove in case you want seller product
    ...
    public function stock_limit_list(Request $request, $type)
    {
        ...
        $pro = $pro->orderBy('id', 'DESC')->where('product_type', "product")->paginate(Helpers::pagination_limit())->appends(['status' => $request['status']])->appends($query_param);
    ...
    {
        $cat = Category::where(['parent_id' => $request->parent_id])->withoutGlobalScope('home_status')->get();
    ...
    public function edit($id)
    {
        ...
        if (in_array('supplier_management', (array)json_decode(auth("admin")->user()->role->module_access)) == true) {
            $product = Product::withoutGlobalScopes()->where("user_id", auth("admin")->id())->whereStatus(false)->with(['translations', 'suppliers'])->find($id);
        } else {
            $product = Product::withoutGlobalScopes()->with(['translations', 'suppliers'])->find($id);
        }
        $product_category = json_decode($product->category_ids);
        $product->colors = json_decode($product->colors);
        $categories = Category::where(['parent_id' => 0])->withoutGlobalScope('home_status')
            ->get();
        $br = Brand::orderBY('name', 'ASC')->get();
        $suppliers = Supplier::orderBY('name', 'ASC')->get();

        return view('admin-views.product.edit', compact('categories', 'br', 'product', 'product_category', 'suppliers'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->has('supplier_ids') /* &&  count($request->supplier_ids) > 0 */) {
        }
        if ($request->has('choice_no')) {
            foreach ($request->choice_no as $key => $no) {
                $name = 'choice_options_' . $no;
