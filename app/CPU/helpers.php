...
class Helpers
{
    ...
    public static function get_exceprt($string, $length = 200, $more = "[...]")
    {
 ...
 */
        $postdata = [
            ...
            "notification" => [
                ...
                "image" =>  $data['image'] ?? null,
                "order_id" => $data['order_id'] ?? null,
                "title_loc_key" => $data['order_id'] ?? null,
                "is_read" => 0,
                "icon" => "new",
                "sound" => "default"
            ]
    ...
    public static function set_symbol($amount)
    {
        ...
        if (!is_null($position) && $position == 'left') {
            $string = currency_symbol() . '' . self::convertToArabic($amount);
        } else {
            $string = self::convertToArabic($amount) . '' . currency_symbol();
        }
        return $string;
    }

    public static function convertToArabic($amount,)
    {
        $string = number_format($amount, 2);

        $isRtl = session("direction", 'ltr') == "rtl";

        if (!$isRtl) return $string;

        $string = number_format($amount, 2, ",", ".");

        return strtr($string, array('0' => '۰', '1' => '۱', '2' => '۲', '3' => '۳', '4' => '٤', '5' => '٥', '6' => '٦', '7' => '٧', '8' => '۸', '9' => '۹'));
    }
...
if (!function_exists('currency_symbol')) {
    function currency_symbol()
    {
        ...
        if (\session()->has('currency_symbol')) {
            ...
            $translations = session('system_default_currency_info')->load("translations")->translations;
            session()
                ->put('currency_translation', $translations);
            $system_default_currency_info = \session('system_default_currency_info');
            ...
            session()
                ->put('currency_translation', $translations);
            $currency_translation = \session('currency_translation');

            $symbol = $currency_translation?->firstWhere("locale", session("local"))
                ?->firstWhere("key", "symbol")
                ->value ?? $system_default_currency_info->symbol;
